...
use self::threadpool::ScopedPool;
...
fn phase1(mut a: &mut [u8], mut alpha_size: u8) -> usize {
    let pool = ScopedPool::new(THREADS as u32);
    let mut len = a.len();
    ...
    while len > 0 {
        ...
        let slice_width = (len - 1) / THREADS + 1;
        let mut boundary_value = [0; THREADS];

        let chunk_count = THREADS;

        for bvi in 0..chunk_count - 1 {
            boundary_value[bvi] = a[..len][slice_width * (bvi + 1)];
        }

        for (ti, a_part) in a[..len].chunks_mut(slice_width).enumerate() {
            pool.execute(move || {
                // reduce alphabet
                for i in 1..a_part.len() {
                    a_part[i-1] = label(a_part[i-1], a_part[i]);
                }
                if ti < chunk_count - 1 {
                    let n = a_part.len() - 1;
                    a_part[n] = label(a_part[n], boundary_value[ti]);
                }
            });
        }
...
fn phase2(a: &mut [u8]) {
    ...
    for n in 3..6 {
        ...
        for i in 0..a.len() {
            if a[i] == n {
                let left  = if i > 0           { Some(a[i-1]) } else { None };
                let right = if i < a.len() - 1 { Some(a[i+1]) } else { None };
